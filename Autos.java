...
public final class Autos {
  ...
  /*
  public static final Command alignToTarget(CANDriveSubsystem driveSubsystem, String limelight) {
    return driveSubsystem.run(() -> {
      double tx = LimelightHelpers.getTX(limelight);
      driveSubsystem.driveArcade(() -> 0, () -> Math.signum(tx) * .3);
    }).until(() -> Math.abs(LimelightHelpers.getTX(limelight)) < 1.0);
  }
  */
  public static final Command alignToTarget(CANDriveSubsystem driveSubsystem, String limelight) {

    return driveSubsystem.driveArcade(
        () -> 0, 
        () -> {
            double tx = LimelightHelpers.getTX(limelight);
            return Math.signum(tx) * 0.3;
